...
Auto <- read_excel("Export/Auto.xlsx")
...
sentiment <- as.character(Auto$word) #convertire colonna commenti in un vettore di char
...
dataset_sentiment <- cbind(Auto$word, etichette)
...
write.xlsx(dataset_sentiment,"SentimentAnalysis_SingleWord_Mauto.xlsx")


# calcolo sentiment recensione dominante


SentimentAnalysis_SingleWord_Mauto$SENTIMENT <- colnames(SentimentAnalysis_SingleWord_Mauto)[apply(SentimentAnalysis_SingleWord_Mauto, 1, which.max)]


# controllo per eliminare "finta" etichetta "Rabbia" come sentiment dominante
for(i in 1:nrow(SentimentAnalysis_SingleWord_Mauto)){
  if(SentimentAnalysis_SingleWord_Mauto$SENTIMENT[i] == "Rabbia" & SentimentAnalysis_SingleWord_Mauto$Rabbia[i] == 0) {
    SentimentAnalysis_SingleWord_Mauto$SENTIMENT[i] <- "NA"
  }
}


# secondo export per recensioni aggregate
write.xlsx(SentimentAnalysis_SingleWord_Mauto,"SentimentDominante.xlsx")
...
barplot(colSums(etichette),
        ...
        ylab = "Valore",
        main = "Museo Automobile - Sentiment Analysis")
...
# - IMPORTARE DA PYTHON EXCEL RECENSIONI LEMMATIZZATE -
Mauto_lemm <- read_excel("Export/Mauto_lemm.xlsx")

#creazione dataset di partenza
a_corp <- as.character(Mauto_lemm$testo) #trasformare colonna del testo in vettore carattere
...
assoc<-as.data.frame(findAssocs(dat.TDM, "torino", corlimit = 0.1))
